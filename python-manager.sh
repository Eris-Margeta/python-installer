...
function install_python {
  ...
  wget --progress=bar:force:noscroll $PYTHON_URL
  if [ $? -ne 0 ]; then
  ...
  read -p "This will remove $PYTHON_TAR and $PYTHON_DIR: " CLEAN_UP
...
function show_menu {
  ...
  echo "3. Exit"
  echo "========================================"
  read -p "Please select an option [1-3]: " MENU_OPTION

  case $MENU_OPTION in
  1)
    read -p "Enter the Python version you want to install (e.g., 3.10.0): " PYTHON_VERSION
    ...
    ;;
  2)
    read -p "Enter the Python version you want to remove (e.g., 3.10.0): " PYTHON_VERSION
    if [[ ! $PYTHON_VERSION =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
      echo "Invalid version format. Please use format like 3.10.0"
      read -p "Press Enter to continue..."
      show_menu
